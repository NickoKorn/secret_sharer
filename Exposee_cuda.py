...
class ExposureMetric:
    ...
    def train_LLM(self, trainFormat: str, numberOfCanaryInsertion: int, pushOrPopCanary: str, shuffle: int):
        ...
        for epoch in range(epochs):
            ...
            for i in range(n_batches):
                ...
                output, hidden = self.model(batch_in.cuda(), hidden)
                loss = criterion(output, batch_out.cuda())
                loss.backward()
            ...
            with torch.no_grad():
                for i in range(val_batches):
                    ...
                    output_val, _ = self.model(batch_in_val.cuda(), hidden_val)
                    loss_val = criterion(output_val, batch_out_val.cuda())
        ...
        #Linux:
        torch.save(self.model.state_dict(), 'models/charLSTM.pth')

    def trainWithCanaries(self):
        
        #model = CharLSTM()
        self.model.load_state_dict(torch.load('/Users/nicko_kornetzky/Desktop/projects/secret_sharer-main/models/charLSTM.pth', weights_only=True))
        pass
    ...
    def calculatePerplexity(self, trainFormat: str, testFormats: list):
        ...
        self.model.load_state_dict(torch.load('models/charLSTM.pth', weights_only=True))
        self.model.to(self.device)
        ...
        with torch.no_grad():
            ...
            output, hidden = self.model(sequenceX.cuda(), hidden)
            ...
            criterion = nn.CrossEntropyLoss()
            loss = criterion(output.cuda(), sequenceY.cuda())
            print('loss:')
            print(loss)
            ...
            for i in range(len(testFormats)):
                ...
                output, hidden = self.model(sequencesX[i].cuda(), hidden)
                ...
                criterion = nn.CrossEntropyLoss()
                loss = criterion(output.cuda(), sequencesY[i].cuda())
                print('loss:')
                print(loss)
